...
export function resolveCredentialsPath(explicit: string | undefined, exists: (path: string) => boolean = existsSync): string | undefined {
...
export function validateCredentials(credentialsPath: string | undefined): void {
  if (!credentialsPath) {
    throw new Error(
      "Google service account credentials are not configured. Set GOOGLE_APPLICATION_CREDENTIALS or SEO_MCP_CREDENTIALS, or start seo-mcp with --credentials /absolute/path/key.json. The seo_audit and pagespeed tools do not require service account credentials.",
    );
  }
  try {
