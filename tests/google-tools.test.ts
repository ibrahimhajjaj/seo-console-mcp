...
describe("Google-backed tool operations", () => {
  ...
  it("says search opportunities came from the top rows when the window held more", async () => {
    ...
    const rows = Array.from({ length: 5001 }, (_, index) => ({
      keys: [`query ${index}`, "https://example.com/guide"],
      clicks: 1,
      impressions: 500,
      ctr: 0.002,
      position: 8,
  ...
  it("compares current search performance with the preceding equal window", async () => {
    ...
    expect(output.structuredContent).toMatchObject({
      ...
      gainers: [{ keys: ["seo"], clicksDelta: 7 }],
      losers: [],
  ...
  it("finds rows whose CTR trails peers at the same position", async () => {
    ...
    expect(output.structuredContent).toMatchObject({ gaps: [{ keys: ["weak snippet"], expectedCtr: 0.1, missedClicks: 90 }] });
  });
  ...
  it("lists Search Console properties", async () => {
    const clients = fakeClients();
    vi.mocked(clients.searchConsole.sites.list).mockResolvedValue({
      data: {
        siteEntry: [
  ...
  it("queries search analytics with defaults and formats ranked rows", async () => {
    const clients = fakeClients();
    vi.mocked(clients.searchConsole.searchanalytics.query).mockResolvedValue({ data: { rows: [{ keys: ["seo mcp"], clicks: 12, impressions: 100, ctr: 0.12, position: 3.456 }] } });
  ...
  it("escapes delimiters and newlines only in search analytics text", async () => {
    ...
    expect(result.structuredContent).toMatchObject({ rows: [{ keys: { query: "buy cats | dogs" } }, { keys: { query: "first\nsecond" } }] });
  });
  ...
  it("rolls up indexed and not-indexed sitemap URLs", async () => {
    ...
    const output = await indexCoverage(
      clients,
      {
        siteUrl: "sc-domain:example.com",
  ...
  it("isolates URL inspection failures", async () => {
    ...
    expect(output.structuredContent.results).toContainEqual(
      expect.objectContaining({
  ...
  it("resubmits the sitemap when checked URLs are not indexed", async () => {
    ...
    const output = await requestRecrawl(
      clients,
      {
        siteUrl: "sc-domain:example.com",
        sitemapUrl: "https://example.com/sitemap.xml",
  ...
  it("does not resubmit the sitemap on a recrawl dry run", async () => {
    const clients = fakeClients();
    vi.mocked(clients.searchConsole.urlInspection.index.inspect).mockResolvedValue({
      data: { inspectionResult: { indexStatusResult: { verdict: "NEUTRAL", coverageState: "Crawled - currently not indexed" } } },
    });
  ...
  it("checks explicit URLs without a sitemap and reports there is nothing to resubmit", async () => {
    const clients = fakeClients();
    vi.mocked(clients.searchConsole.urlInspection.index.inspect).mockResolvedValue({
      data: { inspectionResult: { indexStatusResult: { verdict: "NEUTRAL", coverageState: "Discovered - currently not indexed" } } },
    });
  ...
  it("extracts PageSpeed field data, category scores, and opportunities", async () => {
    ...
    vi.mocked(clients.pageSpeed.pagespeedapi.runpagespeed).mockResolvedValue({
      data: {
        ...
        lighthouseResult: {
          categories: { performance: { score: 0.91 }, seo: { score: 0.95 } },
          audits: {
