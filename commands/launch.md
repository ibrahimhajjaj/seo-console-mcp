...
1. Run `seo_audit` and `pagespeed` on the key pages (start with the homepage), and `crux_field_data` for real-user Core Web Vitals once the origin has enough traffic to appear in the Chrome UX Report.
2. Run `index_coverage` on the sitemap (or `inspect_url` on a specific page) to confirm Google can index the pages.
3. Run `list_sitemaps` to confirm a sitemap is submitted. Offer to run `submit_sitemap` if it is missing, `request_recrawl` if key pages are not indexed yet, and `indexnow_submit` to notify Bing/Yandex-family engines (it needs an IndexNow key hosted on the site).
