...
7. Run `snapshot` with an `outPath` so this triage is recorded. If an earlier snapshot exists, run `compare_snapshots` against it first and read the deltas as evidence, remembering it reports arithmetic and not cause.
